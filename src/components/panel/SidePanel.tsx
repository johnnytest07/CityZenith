...
import { PDRightsChecker } from "./PDRightsChecker";
import { ObligationEstimator } from "./ObligationEstimator";
...
function computeScore(items: InsightItem[]) {
  ...
  const total = Math.round(
    ((high * 10 + medium * 6 + low * 3) / (items.length * 10)) * 100,
  );
...
export function SidePanel() {
  ...
  const { insight, insightsReport, isLoading, generateInsights } =
    useInsights();
                  ...
                  <>
                    <div className="flex items-center justify-between mb-2.5">
                      <div className="flex items-baseline gap-1">
                        <span
                          className={`text-3xl font-bold tabular-nums leading-none ${colors.text}`}
                        >
                          {score.total}
                        </span>
