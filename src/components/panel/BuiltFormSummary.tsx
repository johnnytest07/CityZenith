...
export function BuiltFormSummary() {
  ...
  for (const f of landuse.features) {
    const tag = (f.properties?.landuse ??
      ...
      {noData ? (
        <>
          <p className="text-gray-600 text-xs mt-2">
            No built form data loaded for this area yet. Pan to zoom level 14+
            and reselect.
          </p>
          <InsightCallout
            ...
            <div className="flex gap-2 text-xs flex-wrap">
              ...
              {heightStats && (
                <>
                  <StatChip
                    label="Min"
                    value={`${heightStats.min.toFixed(1)}m`}
                  />
                  <StatChip
                    label="Max"
                    value={`${heightStats.max.toFixed(1)}m`}
                  />
                  <StatChip
                    label="Mean"
                    value={`${heightStats.mean.toFixed(1)}m`}
                  />
                  <StatChip
                    label="Median"
                    value={`${heightStats.median.toFixed(1)}m`}
                  />
                </>
              )}
              {!heightStats && buildingCount > 0 && (
                <span className="text-gray-600 text-xs">
                  Height data not available in tiles
                </span>
              )}
            </div>
          </div>
