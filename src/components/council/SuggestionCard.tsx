...
export function SuggestionCard({ suggestion }: SuggestionCardProps) {
  const { selectedSuggestionId, setSelectedSuggestion, setHoveredSuggestion } =
    useCouncilStore();
  ...
  // Auto-expand when selected from the map
  useEffect(() => {
      ...
      <div className="p-3">
        ...
        {suggestion.problem && (
          <div className="flex gap-1.5 bg-amber-950/30 border border-amber-800/30 rounded-lg px-2.5 py-1.5 mb-2">
            <span className="text-amber-400 shrink-0 text-[10px] mt-0.5">
              ⚠
            </span>
            <p className="text-[11px] text-amber-200/80 leading-snug">
              {renderText(suggestion.problem)}
            </p>
          </div>
        )}
          ...
          {suggestion.overallOutcome && (
            <div className="mt-3 pt-2 border-t border-gray-800">
              ...
              <p className="text-[11px] text-green-300/80 leading-relaxed">
                {renderText(suggestion.overallOutcome)}
              </p>
            </div>
          )}
